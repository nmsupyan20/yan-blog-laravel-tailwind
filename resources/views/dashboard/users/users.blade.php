<x-dashboard.dashboard>
    ...
    <div class="table-wrapper overflow-x-auto mt-4 shadow-md rounded-lg">
        @php
            $titleTables = ["No.", "Name", "Username", "Email", "Created_at", "Updated_at", "Action"]    
        @endphp
        ...
        <table class="w-full border-solid border border-purple-100">
            ...
            @foreach ($users as $key => $user)
                <tr class="bg-white text-center border-b border-gray-100 border-solid shadow-sm">
                    ...
                    <td class="p-3"> {{ $user->updated_at }} </td>
                    <td class="p-3">
                        <a href="/users/{{ $user->id }}/edit" class="bg-yellow-400 text-black py-2 px-3 rounded-lg">Edit</a>
                        <form action="/users/{{ $user->id }}" method="post" class="inline">
                            ...
                            <button type="submit" class="bg-red-500 text-white py-2 px-3 rounded-lg" name="delete"
                                onclick="return confirm('Anda yakin mau menghapus user ini?')">Delete</button>
                        </form>
                    </td>
                </tr>
            @endforeach
